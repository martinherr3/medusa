...
{
    public class NHibernateHbm : EntityTemplate
    {
        ...
        public override void ProduceCode()
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                {
                    WriteLine(@"    <composite-id name=""ID"" class=""{0}.Domain.{1}+DomainObjectID"">", Helper.PascalCase(Project.Code), Helper.MakeSingle(Table.Code));
                    foreach (ColumnSchema column in primaryKeyColumns)
                    {
                        WriteLine(@"        <key-property type=""{0}"" name=""{1}"" column=""{2}"" />", column.NetDataType, Helper.PascalCase(column.Code), column.Name);
                    }
