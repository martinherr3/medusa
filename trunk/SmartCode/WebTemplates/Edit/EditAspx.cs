...
{
    public class EditAspx : TemplateBase
    {
        public EditAspx()
        {
        ...
        public override void ProduceCode()
        {
            ...
            {
                WriteLine(@"<%@ Page Language=""C#"" AutoEventWireup=""true"" CodeBehind=""{0}Edit.aspx.cs"" Inherits=""{1}.Web.{0}Edit"" %>", Helper.ClassName(Entity.Code), Helper.PascalCase(Domain.Code));
                ...
                foreach (ColumnSchema column in Table.Columns())
                {
                    if (!column.IsIdentity && column.NetDataType != "System.Byte[]" && column.IsRequired && !(column.Control is SmartCode.Model.Profile.CheckBox ))
                    {
